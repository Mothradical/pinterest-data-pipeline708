...
The aim of this project is to design a data pipeline for batch and streaming data, utilising Apache Kafka, Amazon Web Services' cloud technology and Databricks.
...
- AWK Kinesis - Part of AWS, this service processes streaming data. In this project, data were streamed to to Kinesis before being read into Databricks for cleaning and analysis.
...
- Kafka REST Proxy - Provices a RESTful interface to an Kafka cluster, allowing for administrative tasks to be performed on the cluster without the use of native Kafka protocol or clients.
...
load_kinesis_to_databricks.py: This file is used to read streaming data from AWS Kinesis, then transforming and cleaning that data before uploading it to Delta tables.
...
.gitignore: This file indicates the credentials files which could not be shared for security purposes.
...
### 2. Familiarise yourself with the data structure
...
- Allow the REST proxy to perform IAM authentication to the MSK cluster by modifying the kafka-rest.properties file with 'nano kafka-rest.properties'. You will need your configure you access role arn, Boostrap server string, and Plaintext Apache Zookeeper connection string in this file.
...
- Mount your S3 bucket to databricks. My account was set-up to have the neccessary permissions to do this.
...
- If user_posting_emulation.py were to run continuously, the whole process would now emulate batch data processing. New data are uploading continuously, then analysed in intervals.

### AWS Kinesis

- With batch processing done, it's time for stream processing.
- My account was granted permission to use PutRecord and describe an existing Kinesis stream. You will need these permissions to continue.
- You will need the arn of a IAM role with Kinesis access.
- Return the the API you created earlier and allow it to invoke Kinesis actions. You may need to create an IAM role for API access to Kinesis, though I was already provided with one. Cope its arn.
